...
#[inline]
pub(crate) unsafe fn simd_eq_i64_vv(lhs: *const i64, rhs: *const i64, out: *mut bool, len: usize) {
...
/// - out must point to valid memory with at least `len` elements
#[inline]
pub(crate) unsafe fn simd_eq_i64_vc(vec: *const i64, constant: i64, out: *mut bool, len: usize) {
    const LANES: usize = 4;
    let chunks = len / LANES;
...
pub(crate) unsafe fn kernel_eq_i64(
    ...
    match (
        ...
        _ => {
            scalar_eq_i64(lhs, rhs, out_ptr, out_selection, len);
        }
    }
}
