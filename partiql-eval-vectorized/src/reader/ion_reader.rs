...
/// Build schema from projection specification (shared by both readers)
fn build_schema_from_projection(spec: &ProjectionSpec) -> SourceTypeDef {
...
/// Validate projection spec contains only FieldPath sources (shared validation)
fn validate_projection_spec(spec: &ProjectionSpec, reader_name: &str) -> Result<(), EvalError> {
...
// ============================================================================
// BINARY ION READER (Symbol ID-based)
// ============================================================================
...
// ============================================================================
// TEXT ION READER (String-based field names)
// ============================================================================

/// High-performance streaming Ion text reader using string-based field lookups
///
/// # Key Differences from PIonReader (Binary)
/// - Uses `field_name()` which returns `String` instead of symbol IDs
/// - Direct string-based lookups (no symbol table)
/// - Optimized for text Ion format (.ion files)
///
/// # Performance Characteristics
/// - String allocations for field names on each field access
/// - FxHashMap<String, usize> for fast string → vector index lookups
/// - Still avoids repeated column_mut() calls in hot loop
///
/// # Usage
///
/// ```rust,ignore
...
///
/// // Stream from text Ion file
/// let mut reader = PIonTextReader::from_ion_file("data.ion", 1000)?;
/// ```
pub struct PIonTextReader<'a> {
    /// Streaming Ion reader
    reader: ion_rs::Reader<'a>,
    /// Configured projection specification
    projection: Option<ProjectionSpec>,
    ...
    /// Track if we've reached end of stream
    eof_reached: bool,
    /// Maps field names to vector indices for direct string lookups
    field_to_vector_map: Option<FxHashMap<String, usize>>,
}

impl<'a> PIonTextReader<'a> {
    /// Create a new text Ion reader from a BufRead source
    pub fn from_reader<R: BufRead + ToIonDataSource + 'a>(reader: R, batch_size: usize) -> Result<Self, EvalError> {
        let ion_reader = ReaderBuilder::new().build(reader).map_err(|e| {
            BatchReaderError::data_source(DataSourceError::initialization_failed(
                "IonText",
                &format!("Failed to create Ion text reader: {}", e),
            ))
        })?;
    ...
    /// Read next batch_size elements using string-based field name lookups
    /// 
    /// Returns the actual number of rows read (may be less than batch_size at end of stream)
    fn read_elements_batch_with_strings(
        reader: &mut ion_rs::Reader,
        field_map: &FxHashMap<String, usize>,
        batch: &mut VectorizedBatch,
        batch_size: usize,
        ...
        for row_idx in 0..batch_size {
            match reader.next().map_err(|e| {
                ...
                ion_rs::StreamItem::Value(_) => {
                    ...
                    // Inner hot loop: Process fields using string field names
                    loop {
                        match reader.next().map_err(|e| {
                            EvalError::General(format!("Error reading struct field: {}", e))
                        })? {
                            ion_rs::StreamItem::Value(_) => {
                                // TEXT ION PATH: Get field name as string
                                let field_name = reader.field_name().unwrap();
                                let field_text = field_name.text().unwrap();
                                if let Some(&vector_idx) = field_map.get(field_text) {
                                    let value = reader.read_i64().unwrap();
                                    let slice = vector_refs.get_mut(&vector_idx).unwrap();
                                    slice[row_idx] = value;
                                }
    ...
    /// Create a new text Ion reader from a file path (streaming from disk)
    pub fn from_ion_file(path: impl AsRef<Path>, batch_size: usize) -> Result<Self, EvalError> {
        let file = File::open(path.as_ref()).map_err(|e| {
            BatchReaderError::data_source(DataSourceError::initialization_failed(
                "IonText",
    ...
    /// Create a new text Ion reader from bytes (for testing with small data)
    pub fn from_ion_bytes(bytes: &[u8], batch_size: usize) -> Result<Self, EvalError> {
        let cursor = std::io::Cursor::new(bytes.to_vec());
        Self::from_reader(cursor, batch_size)
    }
...
impl<'a> BatchReader for PIonTextReader<'a> {
    fn open(&mut self) -> Result<(), EvalError> {
        Ok(())
    }
    ...
    fn set_projection(&mut self, spec: ProjectionSpec) -> Result<(), EvalError> {
        // Validate projection using shared helper
        validate_projection_spec(&spec, "IonTextReader")?;
        ...
        // Build and cache schema using shared helper
        let schema = build_schema_from_projection(&spec);
        self.cached_schema = Some(schema.clone());
        self.reusable_batch = Some(VectorizedBatch::new(schema, self.batch_size));
        self.projection = Some(spec);
        Ok(())
    ...
    fn next_batch(&mut self) -> Result<Option<VectorizedBatch>, EvalError> {
        ...
        let batch = self.reusable_batch.as_mut()
            .ok_or_else(|| EvalError::General("Reusable batch not initialized".to_string()))?;

        let field_map = self.field_to_vector_map.as_ref()
            .ok_or_else(|| EvalError::General("field_to_vector_map not initialized".to_string()))?;

        // Read batch using string-based field lookups
        let rows_read = Self::read_elements_batch_with_strings(
            &mut self.reader,
            field_map,
            batch,
            self.batch_size,
