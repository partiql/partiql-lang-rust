...
fn arb_tuple_data() -> impl Strategy<Value = Vec<Value>> {
    prop_oneof![
        ...
        prop::collection::vec(
            (-100i64..100i64, "[a-zA-Z]{1,5}").prop_map(|(i, s)| {
        ...
        prop::collection::vec(
            prop_oneof![
                // Complete tuple
                (-50i64..50i64, "[a-zA-Z]{1,3}").prop_map(|(i, s)| {
                    Value::Tuple(Box::new(Tuple::from([
                        ("id", Value::Integer(i)),
                        ("name", Value::String(Box::new(s))),
...
mod unit_tests {
    ...
    fn test_tuple_conversion_various_to_string() {
        ...
        let projections = vec![
            ...
            Projection::new(
                ProjectionSource::FieldPath("bool_val".to_string()),
                2,
                LogicalType::String,
            ),
            Projection::new(
                ProjectionSource::FieldPath("string_val".to_string()),
                3,
                LogicalType::String,
            ),
        ];
