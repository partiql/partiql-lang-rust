...
mod unit_tests {
    ...
    fn test_error_message_minimum_requirements() {
        ...
        let errors = vec![
            ...
            ProjectionError::unsupported_source("FieldPath", "ArrowReader", &["ColumnIndex"])
                .to_string(),
        ...
        for error_msg in errors {
            ...
            assert!(
                ...
                "Error message contains debug artifacts: {}",
                error_msg
            );
            assert!(
                !error_msg.contains("{:?}"),
                "Error message contains debug formatting: {}",
                error_msg
            );
        ...
        let data_errors = vec![
            ...
            DataSourceError::corrupted_data("file.parquet", "row 100", "Invalid checksum")
                .to_string(),
        ...
        for error_msg in data_errors {
            assert!(
                error_msg.len() > 15,
                "Data source error message too short: {}",
                error_msg
            );
            assert!(
                error_msg.chars().next().unwrap().is_uppercase(),
                "Data source error not capitalized: {}",
                error_msg
            );
        }
        ...
        let type_errors = vec![
            TypeConversionError::type_mismatch("col", "String", LogicalType::Int64, None)
                .to_string(),
        ...
        for error_msg in type_errors {
            assert!(
                error_msg.len() > 20,
                "Type conversion error message too short: {}",
                error_msg
            );
            assert!(
                error_msg.chars().next().unwrap().is_uppercase(),
                "Type conversion error not capitalized: {}",
                error_msg
            );
        }
    }
    ...
    fn test_error_context_formatting() {
        let error = BatchReaderError::projection(ProjectionError::invalid_spec(
            "test error",
            "test context",
        ))
    ...
    fn test_eval_error_conversion_preserves_information() {
        let original_error = BatchReaderError::data_source(DataSourceError::corrupted_data(
            "test.parquet",
            "row 50",
            "Checksum mismatch",
        ))
    ...
    fn test_parquet_reader_specific_errors() {
        ...
        file.write_all(b"This is definitely not a Parquet file - just plain text")
            .unwrap();
                ...
                Err(error) => {
                    ...
                    assert!(
                        error_msg.contains("Failed to")
                            || error_msg.contains("initialization failed")
                    );
                    assert!(error_msg.contains("[FATAL]"));
                }
                Ok(_) => panic!("Expected error but got Ok"),
    ...
    fn test_debug_parquet_bounds_checking() {
        ...
        assert!(
            result.is_err(),
            "Expected error for column index 5 but got success"
        );
        let error_msg = result.unwrap_err().to_string();
        assert!(
            error_msg.contains("out of bounds"),
            "Error message should contain 'out of bounds': {}",
            error_msg
        );
    }
}
