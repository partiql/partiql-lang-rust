...
fn create_test_parquet_file() -> Result<NamedTempFile, Box<dyn std::error::Error>> {
    ...
    let score_array = Arc::new(Float64Array::from(vec![
        95.5, 87.2, 92.8, 88.1, 94.3, 
        89.7, 91.4, 86.9, 93.2, 90.1
    ]));
    ...
    let record_batch = RecordBatch::try_new(
        schema.clone(),
        vec![id_array, name_array, score_array, active_array, category_array],
    )?;
...
fn test_type_conversions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let batch = batch.unwrap();
    println!("Type conversions test batch: {} rows, {} columns", batch.row_count(), batch.total_column_count());
...
fn test_field_path_rejection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Err(error) = result {
        ...
        println!("  {}PASS:{} FieldPath correctly rejected: {}", GREEN, RESET, error_msg);
...
fn test_file_not_found() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Err(error) = result {
        let error_msg = format!("{}", error);
        println!("  {}PASS:{} File not found error: {}", GREEN, RESET, error_msg);
    ...
    match result {
        Ok(batch_result) => {
            ...
            if let Err(error) = batch_result {
                let error_msg = format!("{}", error);
                println!("  {}PASS:{} Column bounds error: {}", GREEN, RESET, error_msg);
            }
        }
        Err(_) => {
            // Panic occurred - this is expected for invalid column indices
            println!("  {}PASS:{} Column bounds validation correctly panicked on invalid column index", GREEN, RESET);
        }
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!();
    println!("{}PASS:{} All ParquetReader tests completed successfully!", GREEN, RESET);
