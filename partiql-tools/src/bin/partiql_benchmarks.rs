...
fn main() {
    ...
    let file_based_formats: Vec<_> = data_formats
        .iter()
    ...
    if !file_based_formats.is_empty() {
        ...
        for num_batches in unique_batch_counts {
            ...
            let already_exists = data_formats
                .iter()
                .any(|f| matches!(f, DataFormat::InMemory { num_batches: n } if *n == num_batches));
    ...
    for format in &data_formats {
        println!(
            "    - {} ({} rows)",
            format.name(),
            format_number(format.row_count())
        );
    }
    println!();
    ...
    let queries = vec![
        ...
        BenchmarkQuery {
            ...
            query:
                "SELECT a, b FROM ~input~ WHERE ((a - a + b - b + a - a + b - b) + a % 100000) = 0"
                    .to_string(),
        },
    ];
    ...
    for query in &queries {
        ...
        for format in &data_formats {
            ...
            let result = run_legacy_benchmark(query, format, iterations, warmup_iterations);
        ...
        for format in &data_formats {
            ...
            io::stdout().flush().unwrap();
            let result = run_hybrid_benchmark(query, format, iterations, warmup_iterations);
            if let Some(error) = &result.error {
                println!("✗ ({})", error);
            } else {
                println!("✓ (avg: {:.2}ms)", result.avg_ms);
...
fn run_legacy_benchmark(
    ...
    let (total_rows, path) = match format {
        DataFormat::InMemory { num_batches } => (batch_size * num_batches, None),
        DataFormat::Ion { path } => (count_rows_from_file("ion", path), Some(path.clone())),
        DataFormat::IonBinary { path } => (count_rows_from_file("ionb", path), Some(path.clone())),
    };
...
fn print_cross_engine_analysis(results: &[&BenchmarkResult]) {
    ...
    for size in sizes {
        ...
        let size_results: Vec<_> = results
            .iter()
            .filter(|r| r.data_source_rows == size)
            .copied()
            .collect();

        // Group by data format
        let mut formats: Vec<&str> = size_results
            .iter()
        ...
        let engine_pairs = [("Legacy", "Hybrid")];
...
fn print_cross_format_analysis(results: &[&BenchmarkResult]) {
    ...
    for size in sizes {
        ...
        for engine in &["Legacy", "Vectorized-1", "Vectorized-1024", "Hybrid"] {
            ...
            let file_results: Vec<_> = size_results
                .iter()
            ...
            if let Some(baseline) = baseline {
                ...
                let other_formats: Vec<_> = file_results
                    .iter()
                ...
                for result in other_formats {
                    if baseline.avg_ms > 0.0 && result.avg_ms > 0.0 {
                        ...
                        let overhead_pct =
                            ((result.avg_ms - baseline.avg_ms) / baseline.avg_ms * 100.0).abs();
                        let detail_indent = if multiple_sizes { "       " } else { "     " };

                        if speedup >= 1.0 {
                            println!(
                                "{}{} → {}: {:.2}x faster ({:.0}% overhead reduction)",
                                detail_indent,
                                baseline_name,
                                result.data_format,
                                speedup,
                            ...
                            println!(
                                "{}{} → {}: {:.2}x slower ({:.0}% overhead increase)",
                                detail_indent,
                                baseline_name,
                                result.data_format,
                                1.0 / speedup,
...
fn print_best_case_analysis(results: &[&BenchmarkResult]) {
    ...
    for size in sizes {
        ...
        if let (Some(best_leg), Some(best_vec1)) = (
            best_by_engine.get("Legacy"),
            best_by_engine.get("Vectorized-1"),
        ) {
        ...
        if let (Some(best_leg), Some(best_vec1024)) = (
            best_by_engine.get("Legacy"),
            best_by_engine.get("Vectorized-1024"),
        ) {
...
fn print_scaling_analysis(results: &[&BenchmarkResult]) {
    ...
    for ((engine, format), group) in size_groups.iter_mut() {
        if group.len() > 1 {
            // Sort by row count
            ...
            for (i, result) in group.iter().enumerate() {
                ...
                println!(
                    "     {}M rows: {:.2}ms ({:.0} rows/sec)",
                    result.data_source_rows / 1_000_000,
                    result.avg_ms,
                    throughput
                );
...
fn print_format_comparison_table(results: &[&BenchmarkResult]) {
    ...
    for size in sizes {
        ...
        // Print table rows
        for format in formats {
            let legacy = size_results
                .iter()
                .find(|r| r.engine == "Legacy" && r.data_format == format);
            let vec1 = size_results
                .iter()
                .find(|r| r.engine == "Vectorized-1" && r.data_format == format);
            let vec1024 = size_results
                .iter()
                .find(|r| r.engine == "Vectorized-1024" && r.data_format == format);
            let hybrid = size_results
                .iter()
                .find(|r| r.engine == "Hybrid" && r.data_format == format);
...
fn print_speedup_matrix(results: &[&BenchmarkResult]) {
    ...
    for size in sizes {
        ...
        let mut legacy_configs: Vec<&str> = size_results
            .iter()
        ...
        if legacy_configs.is_empty() || other_configs.is_empty() {
            let indent = if multiple_sizes { "     " } else { "   " };
            println!(
                "{}(Need both Legacy and other engine results for matrix)",
                indent
            );
            if multiple_sizes {
                println!();
        ...
        let row_labels: Vec<String> = legacy_configs
            .iter()
            .map(|fmt| create_label("Legacy", fmt))
            .collect();
        let col_labels: Vec<String> = other_configs
            .iter()
        ...
        for label in &col_labels {
            print!(
                "│{:^width$}",
                truncate_label(label, col_width),
                width = col_width
            );
        }
        println!("│");
        ...
        for (i, legacy_fmt) in legacy_configs.iter().enumerate() {
            ...
            let legacy_result = size_results
                .iter()
                .find(|r| r.engine == "Legacy" && r.data_format == *legacy_fmt);

            for (engine, fmt) in &other_configs {
                let other_result = size_results
                    .iter()
                    .find(|r| r.engine == *engine && r.data_format == *fmt);
    ...
    println!("   Abbreviations: Vec=Vectorized, Hy=Hybrid, Mem=In-Memory, IonB=Ion Binary");
}
...
fn print_usage(program: &str) {
    ...
    println!("                          Supported formats: .ion, .10n");
    println!();
    ...
    println!(
        "  {} --include-mem 5000 --iterations 10 test_data/*.ion",
        program
    );
    println!();
    println!("  # Mix different file types");
    println!(
        "  {} test_data/data_b1024_n1000.ion test_data/data_b1024_n10000.10n",
        program
    );
}
