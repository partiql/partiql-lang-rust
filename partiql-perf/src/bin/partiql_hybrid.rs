...
use partiql_eval::engine::{
    IonRowReaderFactory, PlanCompiler, ReaderFactoryEnum, ScanProvider, InMemGeneratedReaderFactory,
};
...
impl ScanProvider for HybridScanProvider {
    fn reader_factory(
        ...
        match self.data_source.as_str() {
            "mem" => {
                Ok(ReaderFactoryEnum::InMem(InMemGeneratedReaderFactory::new(self.total_rows)))
            }
            "ion" | "ionb" => {
