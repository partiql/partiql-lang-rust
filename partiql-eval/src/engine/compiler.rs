...
fn linearize(plan: &LogicalPlan<BindingsOp>) -> Result<Vec<&BindingsOp>> {
    ...
    loop {
        let op = plan.operator(current).ok_or_else(|| {
            EngineError::InvalidPlan("missing operator for flow".to_string())
        })?;
        order.push(op);
...
fn parse_limit(limit: &LimitOffset) -> Result<Option<usize>> {
    if limit.offset.is_some() {
        return Err(EngineError::InvalidPlan(
            "offset not supported".to_string(),
        ));
    }
